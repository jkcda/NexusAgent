...
import { readFileContent } from '../apis'
...
const diffGroups = computed((): DiffGroup[] => {
  ...
  for (const line of lastDiff.value) {
    if (line.type === 'same') {
      groups.push({ type: 'same', text: line.text, oldLine, newLine })
      oldLine++; newLine++
    } else if (line.type === 'del') {
      groups.push({ type: 'del', text: line.text, oldLine })
      oldLine++
    } else if (line.type === 'add') {
      groups.push({ type: 'add', text: line.text, newLine })
      newLine++
    }
  }
  return groups
...
async function save() {
  try {
    const resp = await fetch('/api/fs/write', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ filePath: props.filePath, content: editedContent.value }) })
...
function notifyExternalChange(diffSummary?: string) {
  if (diffSummary) {
    ...
    for (const line of diffSummary.split('\n')) {
      const t = line.trim()
      if (t.startsWith('- ') || t.startsWith('-')) parsed.push({ type: 'del', text: (t.startsWith('- ') ? t.slice(2) : t.slice(1)) })
      else if (t.startsWith('+ ') || t.startsWith('+')) parsed.push({ type: 'add', text: (t.startsWith('+ ') ? t.slice(2) : t.slice(1)) })
      else if (t && !t.startsWith('已修改') && !t.startsWith('文件') && !t.startsWith('...')) parsed.push({ type: 'same', text: line })
    }
